...
def convert_model(cfg):
    ...
    model_path = f"{root_dir}/outputs/models/trained_model.ckpt"
    ...
    torch.onnx.export(
        ...
        f"{root_dir}/outputs/models/model.onnx",  # where to save the model (can be a file or file-like object)
        export_params=True,
        opset_version=10,
    ...
    logger.info(
        f"Model converted successfully. ONNX format model is at: {root_dir}/outputs/models/model.onnx"
    )
